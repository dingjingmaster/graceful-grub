...
static grub_disk_addr_t grub_affs_read_block (grub_fshelp_node_t node, grub_disk_addr_t fileblock)
{
    ...
    if (!node->block_cache) {
        node->block_cache = grub_malloc (((grub_be_to_cpu32 (node->di.size)
					 >> (9 + node->data->log_blocksize)) / data->htsize + 2) * sizeof (node->block_cache[0]));
        if (!node->block_cache)
            return -1;
    ...
    for (curblock = node->last_block_cache + 1; curblock < target + 1; curblock++) {
        grub_disk_read (data->disk,
                        (((grub_uint64_t) node->block_cache[curblock - 1] + 1) << data->log_blocksize) - 1,
                        GRUB_DISK_SECTOR_SIZE - GRUB_AFFS_FILE_LOCATION, sizeof (file), &file);
        if (grub_errno)
            return 0;
    ...
    grub_disk_read (data->disk,
                    (grub_uint64_t) node->block_cache[target] << data->log_blocksize,
                    GRUB_AFFS_BLOCKPTR_OFFSET + (data->htsize - mod - 1) * sizeof (pos), sizeof (pos), &pos);
    if (grub_errno)
        return 0;
...
static struct grub_affs_data * grub_affs_mount (grub_disk_t disk)
{
    struct grub_affs_data *data;
    ...
    for (bsnum = 0; bsnum < AFFS_MAX_SUPERBLOCK + 1; bsnum++) {
        /* Read the bootblock.  */
        grub_disk_read (disk, bsnum, 0, sizeof (struct grub_affs_bblock), &data->bblock);
        if (grub_errno)
            goto fail;

        /* Make sure this is an affs filesystem.  */
        /* Test if the filesystem is a OFS filesystem.  */
        if (grub_strncmp ((char *) (data->bblock.type), "DOS", 3) != 0 || !(data->bblock.flags & GRUB_AFFS_FLAG_FFS))
            continue;
         ...
         */
        if (!rootblock)    rootblock = grub_malloc (GRUB_DISK_SECTOR_SIZE << AFFS_MAX_LOG_BLOCK_SIZE);
        if (!rootblock)    goto fail;
         ...
         */
        for (log_blocksize = 0; log_blocksize <= AFFS_MAX_LOG_BLOCK_SIZE; log_blocksize++) {
            ...
            /* Read the rootblock.  */
            grub_disk_read (disk,
                            (grub_uint64_t) grub_be_to_cpu32 (data->bblock.rootblock) << log_blocksize,
                            0,
            ...
            if (rblock->type != grub_cpu_to_be32_compile_time (2)
                    || rblock->htsize == 0
                    || currblock[(GRUB_DISK_SECTOR_SIZE << log_blocksize) / sizeof (*currblock) - 1] != grub_cpu_to_be32_compile_time (1))
                continue;

            for (i = 0; i < (GRUB_DISK_SECTOR_SIZE << log_blocksize) / sizeof (*currblock); i++)
...
static char* grub_affs_read_symlink (grub_fshelp_node_t node)
{
    struct grub_affs_data *data = node->data;
    grub_uint8_t *latin1, *utf8;
    const grub_size_t symlink_size = ((GRUB_DISK_SECTOR_SIZE << data->log_blocksize) - GRUB_AFFS_SYMLINK_OFFSET);
    ...
    grub_disk_read (data->disk,
                    ...
                    GRUB_AFFS_SYMLINK_OFFSET, symlink_size, latin1);
    if (grub_errno) {
        grub_free (latin1);
        return 0;
    }
...
static int grub_affs_create_node (grub_fshelp_node_t dir, grub_fshelp_iterate_dir_hook_t hook, void *hook_data,
                                  struct grub_fshelp_node **node, grub_uint32_t **hashtable, grub_uint32_t block, const struct grub_affs_file *fil)
{
        ...
        switch ((*node)->di.type) {
            ...
            case grub_cpu_to_be32_compile_time (GRUB_AFFS_FILETYPE_HARDLINK): {
	            ...
	            err = grub_disk_read (data->disk,
                                      (((grub_uint64_t) (*node)->block + 1) << data->log_blocksize) - 1,
                                      GRUB_DISK_SECTOR_SIZE - GRUB_AFFS_FILE_LOCATION, sizeof ((*node)->di), (char *) &(*node)->di);
	            if (err)
	                return 1;
	            continue;
            }
...
static int grub_affs_iterate_dir (grub_fshelp_node_t dir, grub_fshelp_iterate_dir_hook_t hook, void *hook_data)
{
    unsigned int i;
    struct grub_affs_file file;
    ...
    *node = *dir;
    ...
    grub_disk_read (data->disk,
                    (grub_uint64_t) dir->block << data->log_blocksize, GRUB_AFFS_HASHTABLE_OFFSET,
    ...
    for (i = 0; i < data->htsize; i++) {
        ...
        /**
         * Every entry in the hashtable can be chained.  Read the entire chain.
         */
        next = grub_be_to_cpu32 (hashtable[i]);

        while (next) {
            grub_disk_read (data->disk,
			  (((grub_uint64_t) next + 1) << data->log_blocksize) - 1,
			  GRUB_DISK_SECTOR_SIZE - GRUB_AFFS_FILE_LOCATION, sizeof (file), (char *) &file);
            if (grub_errno)
                goto fail;

            if (grub_affs_create_node (dir, hook, hook_data, &node, &hashtable, next, &file)) {
...
static grub_err_t grub_affs_open (struct grub_file *file, const char *name)
{
    struct grub_affs_data *data;
    ...
    grub_fshelp_find_file (name, &data->diropen, &fdiro, grub_affs_iterate_dir, grub_affs_read_symlink, GRUB_FSHELP_REG);
    if (grub_errno)
        goto fail;
...
static grub_err_t grub_affs_close (grub_file_t file)
{
    struct grub_affs_data *data = (struct grub_affs_data *) file->data;
...
static grub_ssize_t grub_affs_read (grub_file_t file, char *buf, grub_size_t len)
{
    struct grub_affs_data *data = (struct grub_affs_data *) file->data;
...
static grub_int32_t aftime2ctime (const struct grub_affs_time *t)
{
    return grub_be_to_cpu32 (t->day) * 86400 + grub_be_to_cpu32 (t->min) * 60 + grub_be_to_cpu32 (t->hz) / 50 + 8 * 365 * 86400 + 86400 * 2;
}
...
static int grub_affs_dir_iter (const char *filename, enum grub_fshelp_filetype filetype, grub_fshelp_node_t node, void *data)
{
...
static grub_err_t grub_affs_dir (grub_device_t device, const char *path, grub_fs_dir_hook_t hook, void *hook_data)
{
...
static grub_err_t grub_affs_label (grub_device_t device, char **label)
{
    struct grub_affs_data *data;
    struct grub_affs_file file;
    ...
    if (data) {
         ...
         */
         grub_disk_read (data->disk,
                         (((grub_uint64_t) grub_be_to_cpu32 (data->bblock.rootblock) + 1) << data->log_blocksize) - 1,
                         GRUB_DISK_SECTOR_SIZE - GRUB_AFFS_FILE_LOCATION, sizeof (file), &file);
         if (grub_errno)
             return grub_errno;
...
static grub_err_t grub_affs_mtime (grub_device_t device, grub_int32_t *t)
{
    struct grub_affs_data *data;
    grub_disk_t disk = device->disk;
    ...
    *t = 0;
    ...
    grub_disk_read (data->disk,
                    (((grub_uint64_t) grub_be_to_cpu32 (data->bblock.rootblock) + 1) << data->log_blocksize) - 1,
                    GRUB_DISK_SECTOR_SIZE - 40, sizeof (af_time), &af_time);
    if (grub_errno) {
        grub_dl_unref (my_mod);
        grub_free (data);
        return grub_errno;
