...
//    if (verbosity > 1)
    ...
    if (!grub_install_source_directory) {
        if (!target) {
            ...
            if (!t) {
                grub_util_error ("%s", _("Unable to determine your platform. Use --target."));
            }
            target = xstrdup (t);
        }
        grub_install_source_directory = grub_util_path_concat (2, grub_util_get_pkglibdir (), target);
    }
    ...
    if (is_efi) {
        ...
        if (!efidir) {
             ...
             * --root-directory.
             */
             if (!grub_util_is_directory (d) && rootdir && grub_strcmp (rootdir, "/") != 0) {
                 free (d);
                 d = xstrdup (rootdir);
             }
             if (grub_util_is_directory (d))
        ...
        if (!efidir_device_names || !efidir_device_names[0])
            grub_util_error (_("cannot find a device for %s (is /dev mounted?)"), efidir);
        ...
        if (!efidir_grub_devname)
            grub_util_error (_("cannot find a GRUB drive for %s.  Check your device.map"), efidir_device_names[0]);
        ...
        if (grub_strcmp (fs->name, "hfs") == 0 || grub_strcmp (fs->name, "hfsplus") == 0)
            efidir_is_mac = 1;
    ...
    if (platform == GRUB_INSTALL_PLATFORM_POWERPC_IEEE1275) {
        ...
        if (macppcdir) {
            ...
            if (!macppcdir_device_names || !macppcdir_device_names[0])
                grub_util_error (_("cannot find a device for %s (is /dev mounted?)"), macppcdir);
            ...
            if (!macppcdir_grub_devname)
                grub_util_error (_("cannot find a GRUB drive for %s.  Check your device.map"), macppcdir_device_names[0]);
            ...
            if (grub_strcmp (fs->name, "hfs") != 0 && grub_strcmp (fs->name, "hfsplus") != 0 && !is_guess)
                grub_util_error (_("filesystem on %s is neither HFS nor HFS+"), macppcdir);
    ...
    if (!grub_devices || !grub_devices[0])
        grub_util_error (_("cannot find a device for %s (is /dev mounted?)"), grubdir);
    ...
    for (curdev = grub_devices, curdrive = grub_drives; *curdev; curdev++, curdrive++) {
        *curdrive = grub_util_get_grub_dev (*curdev);
        if (! *curdrive)
            grub_util_error (_("cannot find a GRUB drive for %s.  Check your device.map"), *curdev);
    }
    ...
    *curdrive = 0;
    ...
    if (!config.is_cryptodisk_enabled && have_cryptodisk)
        grub_util_error (_("attempt to install to encrypted disk without cryptodisk enabled. "
                           "Set `%s' in file `%s'"), "GRUB_ENABLE_CRYPTODISK=y", grub_util_get_config_filename ());
    ...
    if (install_device) {
        if (install_device[0] == '(' && install_device[grub_strlen (install_device) - 1] == ')') {
	        ...
	        if (!install_drive)
	            grub_util_error (_("cannot find a GRUB drive for %s.  Check your device.map"), install_device);
        }
    }
    ...
    if (!have_abstractions) {
        ...
        || !have_bootdev (platform)) {
            ...
            if (!force_file_id && grub_fs->fs_uuid && grub_fs->fs_uuid (grub_dev, &uuid)) {
                grub_print_error ();
                grub_errno = 0;
                uuid = NULL;
            }
