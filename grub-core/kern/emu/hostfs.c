...
static grub_err_t grub_hostfs_dir (grub_device_t device, const char *path, grub_fs_dir_hook_t hook, void *hook_data)
{
    ...
    if (! dir)
        return grub_error (GRUB_ERR_BAD_FILENAME, N_("can't open `%s': %s"), path, grub_util_fd_strerror ());
...
static grub_err_t grub_hostfs_open (struct grub_file *file, const char *name)
{
    grub_util_fd_t f;
    ...
    if (! GRUB_UTIL_FD_IS_VALID (f))
        return grub_error (GRUB_ERR_BAD_FILENAME, N_("can't open `%s': %s"), name, strerror (errno));
...
static grub_ssize_t grub_hostfs_read (grub_file_t file, char *buf, grub_size_t len)
{
    struct grub_hostfs_data *data;

    data = file->data;
    if (grub_util_fd_seek (data->f, file->offset) != 0) {
        grub_error (GRUB_ERR_OUT_OF_RANGE, N_("cannot seek `%s': %s"), data->filename, grub_util_fd_strerror ());
        return -1;
    }

    unsigned int s = grub_util_fd_read (data->f, buf, len);
    if (s != len)
        grub_error (GRUB_ERR_FILE_READ_ERROR, N_("cannot read `%s': %s"), data->filename, grub_util_fd_strerror ());

    return (signed) s;
}

static grub_err_t grub_hostfs_close (grub_file_t file)
{
    struct grub_hostfs_data *data;
...
static grub_err_t grub_hostfs_label (grub_device_t device __attribute ((unused)), char **label __attribute ((unused)))
{
    *label = 0;
